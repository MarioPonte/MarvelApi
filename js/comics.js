...
async function showModal(clickedElement){
    ...
    document.getElementById("modalComic").innerHTML = '<div class="modal-dialog modal-lg"><div class="modal-content"> <div class="modal-header"> <h5 id="modalComicName" class="modal-title">' + pegueiDadosModal.data.results[clickedElement].title + '</h5> <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button> </div> <div class="modal-body"> <strong><p>Price:</strong> ' + pegueiDadosModal.data.results[clickedElement].prices[0].price + '$</p> <p class="creators"><strong>Creators:</strong> ' + allCreators + '</p> </div> <div class="heroHistory"><div class="comicImgDiv text-center"><img class="comicImgElement" src="' + pegueiDadosModal.data.results[clickedElement].thumbnail.path + '.' + pegueiDadosModal.data.results[clickedElement].thumbnail.extension + '" alt="" srcset=""></img></div><a class="btn btn-primary btnInfoHero" href="' + pegueiDadosModal.data.results[clickedElement].urls[0].url + '" target="_Blank">More Info</a></div> </div> </div> </div> </div>';
